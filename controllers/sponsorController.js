...
class SponsorController {
  ...
  static handleFileUpload = (req, res, next) => {
    upload.single("logo")(req, res, (err) => {
      if (err) {
        return SponsorController.handleError(res, err, "Error uploading file", 400);
      }
      next();
    });
  };
    ...
    try {
      SponsorController.handleFileUpload(req, res, async () => {
        ...
        try {
          ...
          res.status(201).json({ message: "Sponsor added successfully", data: result });
        } catch (err) {
  ...
  static deleteSponsor = async (req, res) => {
    try {
      ...
      res.status(200).json({ message: "Sponsor deleted successfully", data: result });
    } catch (err) {
  ...
  static displayAll = async (req, res) => {
    try {
      ...
      res.status(200).json({ message: "Sponsor list retrieved successfully", data });
    } catch (err) {
